...
.NOTES
  ...
  Version        : 2.2 - Fix PSScriptAnalyzer (orgId, principalId) + proxy + credentials Email
...
# $orgData.OrgId est disponible si besoin futur (non utilise dans les appels API actuels)
...
# Lecture robuste des cles (Email, Login, User, Username)
$siteUrl   = [string]$siteData.SiteUrl
$siteEmail = ""
foreach ($key in @('Email','Login','User','Username')) {
  if ($siteData.ContainsKey($key) -and $siteData[$key]) {
    $siteEmail = [string]$siteData[$key]; break
  }
}

# Lecture robuste du token (ApiTokenSecureString deja un SecureString)
$siteToken = ""
if ($siteData.ContainsKey('ApiTokenSecureString') -and $null -ne $siteData['ApiTokenSecureString']) {
  $ss = $siteData['ApiTokenSecureString'] -as [System.Security.SecureString]
  if ($ss) {
    $siteToken = [System.Net.NetworkCredential]::new("", $ss).Password
  }
} elseif ($siteData.ContainsKey('ApiToken')) {
  $siteToken = [string]$siteData['ApiToken']
}

if (-not $siteEmail -or -not $siteToken) {
  $availableKeys = ($siteData.Keys | Sort-Object) -join ", "
  throw "Credentials incomplets (email='$siteEmail', token vide=$(-not $siteToken)). Cles disponibles : $availableKeys"
}
...
# --- Test de connexion ---
$testResp = Invoke-ApiCall -Method "GET" -Url "${baseUrl}/wiki/rest/api/user/current" -Headers $confluenceHeaders
if ($testResp.ok) {
  try {
    $me = $testResp.content | ConvertFrom-Json
    Log ("Connecte en tant que : {0} ({1})" -f $me.displayName, $siteEmail)
  } catch {
    Log "Connexion OK (impossible de parser le profil)"
  }
} else {
  throw ("Echec de connexion (status {0}) : {1}" -f $testResp.status, $testResp.error)
}
...
foreach ($space in $allSpaces) {
  ...
  # --- Date de creation via API v1 expand=history ---
  $dateCreation = ""
  $encodedKey = [System.Uri]::EscapeDataString($spKey)
  ...
  foreach ($perm in $allPermissions) {
    ...
    $pType       = [string]$principal.type
    $principalId = [string]$principal.id   # FIX: $pId renomme en $principalId (PSAvoidAssignmentToAutomaticVariable)
