<#
Export des membres INACTIFS des groupes Jira dont le nom contient "DSIM" (Jira Cloud)
- TLS 1.2 + Proxy système Windows (PAC/WPAD/McAfee)
- Stockage local chiffré (DPAPI) de l'email+token (pas de ressaisie)
- Exporte 3 CSV:
    * C:\Temp\groupes_DSIM_inactifs.csv
    * C:\Temp\membres_groupes_DSIM_inactifs_detail.csv
    * C:\Temp\membres_groupes_DSIM_inactifs_uniques.csv

Usage:
  .\listeGroupesDsimMembresInactifs.ps1
  .\listeGroupesDsimMembresInactifs.ps1 -Filter "DSIM"
  .\listeGroupesDsimMembresInactifs.ps1 -ResetCreds
#>
...
$exportGroupsPath  = "C:\Temp\groupes_DSIM_inactifs.csv"
$exportDetailPath  = "C:\Temp\membres_groupes_DSIM_inactifs_detail.csv"
$exportUniquePath  = "C:\Temp\membres_groupes_DSIM_inactifs_uniques.csv"
...
# Filtrage local contains (sécurité)
...
# ----------------------------
# 2) Pour chaque groupe => membres, filtrés sur Active = false
# ----------------------------
Write-Host "Récupération des MEMBRES INACTIFS par groupe..." -ForegroundColor Cyan
...
foreach ($g in $groups) {
    ...
    while ($true) {
        $incInactive = "true"   # on veut explicitement tous les users, actifs + inactifs
        ...
        foreach ($u in $values) {
            # On ne conserve QUE les inactifs
...
Write-Host ("Lignes détail membres inactifs: " + $resultDetail.Count) -ForegroundColor Green

# ----------------------------
# 3) Liste unique des personnes inactives + groupes DSIM associés
# ----------------------------
$resultUnique = @()
...
Write-Host "Export CSV groupes (DSIM)         : $exportGroupsPath" -ForegroundColor Cyan
$resultGroups | Export-Csv -Path $exportGroupsPath -NoTypeInformation -Encoding UTF8

Write-Host "Export CSV membres INACTIFS détail: $exportDetailPath" -ForegroundColor Cyan
$resultDetail | Export-Csv -Path $exportDetailPath -NoTypeInformation -Encoding UTF8

Write-Host "Export CSV membres INACTIFS uniq. : $exportUniquePath" -ForegroundColor Cyan
$resultUnique | Export-Csv -Path $exportUniquePath -NoTypeInformation -Encoding UTF8

Write-Host "Terminé." -ForegroundColor Green
Write-Host " - Groupes (DSIM)          : $exportGroupsPath"
Write-Host " - Membres inactifs détail : $exportDetailPath"
Write-Host " - Membres inactifs uniques: $exportUniquePath"
